% Plot de la comparaison des spectres de modes des différents partenaires et de leur dispersion
% spectrale.
% Ce script nécessite la fonction stdft_ et extract_folder_inventory pour être opérationnel.
...
%%% User inputs
lib = {'Lib_algo', 'Lib_UI'};
databasePath = 'F:\Professionnel\Projet\Ongoing\Sherlock_Holmes_V2\Data/20111100_Witschi_DB1/';
signalDuration      = 2;
...
regQuery = 'P01|P02|P03|P04|P05|P06|P07|P08|P09';
% regQuery = 'P01|P04|P09';
...
for iFolder = 1 : nFolder,
    ...
    psdAM = mean(psdA);
    psdAS = std(psdA);
    psdABU = psdAM + 2*psdAS;
    psdABU(psdABU<0) = zeros(1, sum(psdABU<0));
    psdABL = psdAM - 2*psdAS;
    psdABL(psdABL<0) = zeros(1, sum(psdABL<0));
    
%         folderName2 = 'DEFDEFDEF';
...
%     
%     
    if iFolder == 1 || iFolder == 4 || iFolder == 7,
        hGui11 = figure('Position',[0 0 fullscreen(3) fullscreen(4)], ...
              'color', 'white', ...
              'Visible', printFlag);
    end
...
%     title(['Mode spectrum - Mean and dispersion (2 \sigma) - P', folderName2(iFolder)], 'FontWeight', 'bold');
    legend('Moyenne spectrale', 'Ecart-type spectral (2 \sigma)');
...
%     subplot(3, 1, mod(iFolder-1, 3)+1); hold on;
% 
%     plot(stdftFAxis/1e3, psdAM);
%     plot(stdftFAxis/1e3, psdABU, ':k');
%     plot(stdftFAxis/1e3, psdABL, ':k');
% 
%     axis tight;
%     xlabel('Frequency [kHz]');
%     ylabel('Normalized energy magnitude [-]');
% %     title(['Spectrum dispersion - Attacks ', folderName{iFolder}], 'FontWeight', 'bold');
%     title(['Attack spectrum - Mean and dispersion (2 \sigma) - P', folderName2(iFolder)], 'FontWeight', 'bold');
% %     legend('Moyenne spectrale', 'Ecart-type spectral (2 \sigma)');
