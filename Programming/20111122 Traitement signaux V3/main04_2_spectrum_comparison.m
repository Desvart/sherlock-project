...
t = {'Spectrogram without known default', 'Harmonics without known default', 'Attacks without known default',...
    'Spectrogram with "Echappement non lubrifié"', 'Harmonics with "Echappement non lubrifié"', 'Attacks with "Echappement non lubrifié"',...
    'Spectrogram with "Spiral décentré"', 'Harmonics with "Spiral décentré"', 'Attacks with "Spiral décentré"',...
    'Spectrogram with "Palettes sur impulsion"', 'Harmonics with "Palettes sur impulsion"', 'Attacks with "Palettes sur impulsion"', ...
    'Spectrogram with "Complication"', 'Harmonics with "Complication"', 'Attacks with "Complication"'};
...
annotation('textbox', 'string', databasePath(end-6), 'Position', [0, 0, 0.1, 0.1], 'FitBoxToText', 'on');
for i = 1 : nFile/3,
    
    subplot(nFile/3, 1, i);
    plot(stdftFAxis/1e3, psd((i-1)*3+1+(0:2), :)');
    
    axis tight;
    xlabel('Frequency [kHz]');
    ylabel('Energy magnitude [-]');
    title(t{(i-1)*3 + 1}, 'FontWeight','bold');
   
    
end

if shouldPrint == true,
    orient(hGui, 'portrait');
    print(hGui);
end
...
for i = 1 : nFile/3,
    ...
    xlabel('Frequency [kHz]');
    ylabel('Energy magnitude [-]');
    title(t{(i-1)*3 + 2}, 'FontWeight','bold');
...
for i = 1 : nFile/3,
    ...
    xlabel('Frequency [kHz]');
    ylabel('Energy magnitude [-]');
    title(t{(i-1)*3 + 3}, 'FontWeight','bold');
end
