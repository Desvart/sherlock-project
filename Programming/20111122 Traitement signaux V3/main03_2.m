...
%%% User inputs
lib = {'Lib_algo', 'Lib_UI'};
databasePath = '../../Data/20111100_Witschi_DB1/P01/V1.2/';
signalDuration      = 1;
...
for iFile = 8,
    ...
    %% Time-domain analysis - Spectrogram only hamonics
    
    %%% Compute spectrogram
    th = 1.5;
    t = db(median(ptd/max(ptd)));
    s = stdft/max(max(stdft));
    noiseLevel = median(s(ceil(end/2), :));
    s(:, db(ptd/max(ptd))>t/th) = noiseLevel*ones(length(psd), sum(db(ptd/max(ptd))>t/th));
    sH = s;
    o = ones(size(sH));
    sH(db(s)<db(mean(s(ceil(end/2), :)))*4/5) = noiseLevel*o(db(s)<db(mean(s(ceil(end/2), :)))*4/5);
    ...
    ptdSH = sum(s, 1);
        ...
        %% Time-domain analysis - Spectrogram only attacks
    
    %%% Compute spectrogram
    th = 1.5;
    t = db(median(ptd/max(ptd)));
    sA = stdft/max(max(stdft));
    noiseLevel = median(s(ceil(end/2), :));
    sA(:, db(ptd/max(ptd))<t/th) = noiseLevel*ones(length(psd), sum(db(ptd/max(ptd))<t/th));
%     s2 = s;
%     o = ones(size(s2));
%     s2(db(s)<db(mean(s(ceil(end/2), :)))*4/5) = noiseLevel*o(db(s)<db(mean(s(ceil(end/2), :)))*4/5);
...
%                   'color', 'white', ...
%                   'Visible', printFlag, ...
%                   'name', sprintf('Movement: %s', filePathList{iFile}(end-18-7:end-7)));
%     
%     %%% Subplot [1,2] : spectrogram magnitude in dB
%     h1 = subplot(6, 7, [2:7, 9:14, 16:21, 23:28, 30:35]); hold on;
%     
%     mesh(stdftTAxis*1e3, stdftFAxis/1e3, db(stdft/max(max(stdft)))); shading flat;
%     
% %     xlabel('Time [ms]');
%     ylabel('Frequency [kHz]');
%     title(sprintf('Movement: %s\nSpectrogram (N = %d = %2.1f ms @ fs = %d kS/s, S = 1/%d = %2.1f ms @ fs = %d kS/s)', ...
%                                 filePathList{iFile}(end-18-7:end-7), ...
...
%     subplot(6, 7, 1:7:29);
%     
%     plot(db(psd/max(psd)), stdftFAxis/1e3, 'k');
%     
%     set(gca, 'XDir', 'reverse');
%     axis tight;
%     
...
%     subplot(6, 7, 37:42);
%     
%     plot(stdftTAxis*1e3, db(ptd/max(ptd)), 'k');
% 
%     axis tight;
%     
%     xlabel('Time [ms]');
...
%     end
    ...
    %% Time-frequency domain plot - Spectrogram without attack and without hamonics
...
%     ylabel('Frequency [kHz]');
    ...
    %%%
%     subplot(2, 8, 11:16);
%     mesh(stdftTAxis*1e3, stdftFAxis/1e3, db(sH/max(max(sH)))); shading flat;
%     view(2);
    
    %%%
%     subplot(2, 8, 5:8);
%     mesh(stdftTAxis*1e3, stdftFAxis/1e3, db(sA/max(max(sA)))); shading flat;
%     view(2);
 %%% Subplot [1,1] : Power Spectrum Densisty (PSD)
    subplot(1, 8, 1);
    ...
    plot(db(psdSA/max(psdSA)), stdftFAxis/1e3, 'k');
    ...
    xlabel('Energy [dB]');
%     ylabel('Frequency [kHz]');
    title('Attacks spectrum');
    
    %%% Subplot [1,1] : Power Spectrum Densisty (PSD)
    subplot(1, 8, 3);
    
    plot(db(psdSH/max(psdSH)), stdftFAxis/1e3, 'k');
...
%     subplot(6, 7, 37:42);
%     
%     plot(stdftTAxis*1e3, db(ptdS/max(ptdS)), 'k');
% 
%     axis tight;
%     
%     xlabel('Time [ms]');
