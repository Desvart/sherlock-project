...
worldflag=0; %flag pour la classe world a entrainer
if (worldflag==1), rejectflag=1;end;

%3classes
NbreSig=[38 69 58];%Nombre de Signaux : Explosions, Bris de verre, cris
...
repertclasses{1}='/verres/verre';
repertclasses{2}='/portes/porte';
repertclasses{3}='/explos/explo';
...
Msep=2;%Nombre de features apr�s r�duction LDA
...
for i=1:iterations,
   ...
   %Training
	...
	%save training.mat m c v d vsep dsep totmean totvar;
   
   %Testing
 	%load training;
   [membership,meancert,rejmembership]=testing(net,testfiles,NbreSigTest,v,d,vsep,dsep,M,Msep,totmean,totvar,snratiotest,rejectflag);
   ...
   %rejection 
   %with meancert
   cert_threshold=0.795;%0.795
   meanrejmembership=membership;
	certmax=max(meancert);
   meanrejmembership(find(certmax<cert_threshold))=0;
   
   %with cert 
   %utiliser rejmembership directement
   
   indOK=find(rejmembership~=0);%seulement temporal rejection
   %indOK=find(meanrejmembership~=0);%seulement avec meancert rejection
   %indOK=find((rejmembership~=0) & (meanrejmembership~=0));%avec les deux
