...
function [meanMatrix, covMatrix, PCABasis, LDABasis] = Training_Bayes(trainingFile, ...
    ...
    nbTrainingFilePerClass    = trainingFile{1};
    nbTrainingFile = sum(nbTrainingFilePerClass);
    nbClass           = length(nbTrainingFilePerClass);
    ...
    for i = 1 : nbClass,
        ...
        lastId            = sum(nbTrainingFilePerClass(1:i));
    ...
    % This operation is done to remove biaises associated with differently scaled feture
    % values and to preserve good numerical behavior.
    % ------------------------------------------------------------------------------------
