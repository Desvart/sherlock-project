function [feature, chrono] = loadfeature()
% LOADFEATURE   
%
% Synopsis:     loadfeature()
%
% Inputs:   []
%
% Output:   chrono  = Function elapse time (s).

% Notes: 

% Project Sphere: Alpha 0.5.2
% Author: Pitch Corp.  -  2011.02.16  -  Copyleft ;-)
% ------------------------------------------------------------------------------------------------ %

    % --- Launch function chronometer
    chronoTic = tic;

    switch config('isRealData'),
        
        % --- Extract features from sound file coming from database
        case true, 
            rawFeature = extractFeatureFromDatabase(); 
            
        % --- Generate feature create from gaussian models
        case false, 
            rawFeature = generateFeatureFromMathModel(); 
            
        otherwise,
            error('Field ''isRealData'' from configuration file has an unknown value.');
    end
    
    % --- Rearrange features
    feature = featureformatconversion(rawFeature);
...
function feat = extractFeatureFromDatabase()
...
function feature = featureformatconversion(rawFeature)
