...
% ------------------------------------------------------------------------------------------------ %
    ...
    if isempty(config), % Default values
        ...
        config.isDebug               	= true;     % If true, verbose mode is on (= debug mode). This will display additional information during execution process.
        config.shouldSaveProcess       	= true + true;     % If true: 1) config + logging, 2) data.
        config.shouldErasePreviousData 	= true + false;     % If true, previous logging text file are deleted before saving a new one.
...
%         config.configPathToLoad = './Output/20110803_114928_savedConfig.mat'; % Path where is store a config to load.
        config.configPathToLoad = ''; % Path where is store a config to load.
        
        
        % --- Database configuration
        config.databaseName = 'db_Duf_2C_XSmall'; % 
        config.databaseName = 'db_Duf_2C'; % 
        config.databaseName = ''; % 
        config.trainingSnr  = 120;        % (dB)
        config.testingSnr   = 121;        % (dB)
...
%         config.classCov(4,4,3)  = 5;
    ...
    switch nargin,
        ...
        case 0,                             % Get entire structure
            ...
            elseif nargout == 0
                if ~isempty(config.configPathToLoad),
                    config_('load', config.configPathToLoad);
                else
                    if strcmp(config.modelType, 'gmm') && config.gmmNGmm == 1,
                        config.modelType = 'bayes';
                    end
                    if ~isempty(config.databaseName),
                        extraction_ = str2func([config.extractionType, '_extraction0']);
                        featureDim = extraction_();
                    else
                        featureDim = size(config.classMean, 1);
                    end
                    config.nDimPca = min(config.nDimPca, featureDim);
                    config.nDimLda = min(config.nDimLda, featureDim);
                end
            end
