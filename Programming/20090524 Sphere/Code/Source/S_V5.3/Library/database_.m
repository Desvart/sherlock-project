...
% ------------------------------------------------------------------------------------------------ %
    ...
    if isempty(database), % Default values
        ...
        database.trainingFileId         = 0; % Cell(iTrainingId, iClass)
        database.trainingFileLabel      = 0;
        database.trainingFileIdPerClass = {''};
        
        database.nTestingFile           = 0;
        database.nTestingFilePerClass   = 0;
        database.testingFileId          = 0; % Cell(iTrainingId, iClass)
        database.testingFileLabel       = 0;
        database.testingFileIdPerClass  = {''};
            ...
            switch varargin{1},
                ...
                case 'load',                % Set the entire structure
                    if strcmp(varargin{2}, ''),
                        varargin{2} = config_('savedDatabasePath');
                    end
                    if ischar(varargin{2}),
                        if exist(varargin{2}, 'file'),
                            load(varargin{2});
                            database = savedDatabase; %#ok<NODEF>
                        else
                            error('ERROR while loading database : ''%s'' file does not exist.\n\n', varargin{2});
                        end  
                ...
                case 'save',                % Export entire structure into mat-file
                    ...
                    filePath = [config_('outputPath'), varargin{2}, '_savedDatabase.mat'];
                    savedDatabase = database; %#ok<NASGU>
                    save(filePath, 'savedDatabase');
                    output = filePath;
...
function extact_database_metadata()
...
% ------------------------------------------------------------------------------------------------ %
    ...
    db.trainingFileId           = 0;
    db.trainingFileLabel        = 0;
    db.trainingFileIdPerClass   = {''};
	db.testingFileId            = 0;
    db.testingFileLabel         = 0;
    db.testingFileIdPerClass    = {''};
    database_('load', db);
