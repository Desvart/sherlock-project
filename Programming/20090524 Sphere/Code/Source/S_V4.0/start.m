...
%%% Input selection
input.inputFlag = 1; % 0 : features input - 1 : database input
input.inputMode = 2;
...
%%% Add noise to signal (same noise for training and testing sets)
snr = 120; %dB

%%% Feature to extract
featureType = 'spectrogram'; % spectrogram - MFCC
% featureType = 'MFCC'; % spectrogram - MFCC
...
database = databaseQuery(inputFlag, databasePath, snr, featureType);

%%% Init script
[nbCrossvalidation, h] = initScript(database, strategy, nbCrossvalidation);
