...
%     dist222 = [dist222; nan, nan];
    ...
    for iToc = 1 : N_TOCS_PER_FILE,
       ...
       if isnan(maxMes5Idx),
           maxMes5Idx = raw{tocLine, 2} : round(raw{tocLine, 2} + 3.57e-3*fs); % 3.57 is the mean of all distance 5-6
       end
...
% impulseDrop max (6-7) / drop max (5-6)
    ...
    for iToc = 1 : N_TOCS_PER_FILE,
       ...
       if isnan(std5Idx),
           std5Idx = raw{tocLine, 2} : round(raw{tocLine, 2} + 3.57e-3*fs); % 3.57 is the mean of all distance 5-6
       end
    ...
    for iToc = 1 : N_TOCS_PER_FILE,
       ...
       if isnan(e0Idx),
           e0Idx = raw{tocLine, 7} : round(raw{tocLine, 7} + 3.8e-3*fs); % 3.8 is the mean of all distance 7-9
       end
