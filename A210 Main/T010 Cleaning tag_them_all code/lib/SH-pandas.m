function [degagementStartIdxArray, chuteStartIdxArray] = ...
                                    SHH_pandas(signal, stdWinSize, stdRatioThreshold, distThreshold)
% SHH_pandas   Extract the start indexes of the two main impulse of each tocs (namely "degagement" 
% and "chute" which are, respectively, the first and the third theoretical impulse present in a toc).
%
% Inputs :
...
% Project : Sherlock Holmes - PANDAS

% History
% 2.2.b5.1409  Clean comments for release candidate version.
% 2.2.b4.0517   Handle UFO pulse
%   Bug fix :
%       - Add a function (PANDAS_remove_middle_pulse) to remove behavior that detect a third pulse 
%         between "dégagement" and "chute" (the "UFO" one).
%   Fine tuning:
%       - Modify stdRatioThreshold default value from 20 to 15 (adaptation for signal 
%         ML03_R00_B06_N01).
% 2.1.b2.0515   Enhanced hard thresholding poor behavior.
%   Bug fix :
%       - Fixed bug that detect local double peaks in hard thresholding
% 2.0.b0.0510   File restructuration
% 1.0.b0.0220	First functionnal version
% 0.0.a0.0000	File creation (2013.08.10)
...
function [degagementStartIdxArray, chuteStartIdxArray] = ...
                                     PANDAS_remove_middle_pulse(preciseStartIdxArray, distThreshold)
% PANDAS_REMOVE_MIDDLE_PULSE	Detect and remove potential middle pulse between the "dégagement" 
