...
%         axesW    = (100/(nCarKept-1) - 2) / 100; 
        ...
        axesXPos = ((iCar1-2)*(100/(nCarKept-1)) + 4 ) / 100;
...
%         nGauss   = carRange(1) : (carRange(2)-carRange(1))/10000 : carRange(2);
        ...
        for iClass = 1 : nClasses,
            ...
            k1(iClass) = plot(cloudArr(:, 1), cloudArr(:, 2), [col(iClass), classTag(iClass)]);
            k2(iClass) = plot(meanVec(1), meanVec(2), ['g', classTag(iClass)]);
            hEll(iClass) = plot_gaussian_ellipsoid(meanVec, covArr);
            set(hEll(iClass), 'Color', col(iClass));
        end
...
xlabel('Caract�ristique virtuelle 1');
ylabel('Caract�ristique virtuelle 2');
legend([k1, k2, hEll], 'Classe 1', 'Classe 2', 'Centre de gravit� classe 1', 'Centre de gravit� classe 2', 'Dispersion classe 1', 'Dispersion classe 2');
