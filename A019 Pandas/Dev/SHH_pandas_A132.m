function [degagementStartIdxArray, chuteStartIdxArray] = ...
                               SHH_pandas_A132(signal, stdWinSize, stdRatioThreshold, distThreshold)
% SHH_PANDAS   Extract the start indexes of the two main impulse of each tocs (namely "degagement" 
% and "chute" which are, respectively, the first and the third theoretical impulse present in a 
% toc).
%
% Inputs :
...
% Subfunctions : PANDAS_detecte_precise_start_idx, PANDAS_remove_middle_pulse
...
% Log
% 2013.01.22    [A1.3.2] Change 'movingstd' function for my own function ('moving_std') and fix 
%               some typo.
% 2013.01.21    [B1.0][A1.3.1] Test over database ViSo "Barillet" : test passed !
% 2013.01.17    [A1.3.0] Bug fix and fine tuning
...
%                 ML03_R00_B06_N01).
% 2013.01.15    [A1.2.0] Bug fixes 
%               - Modify peak detection to avoid near double peaks detection with an "hard" 
%                 thresholding.
% 2013.01.10    [A1.0.0] File restructuration.
% 2012.08.10	[A0.0.0] File creation.
...
% See also movingstd
    ...
    stdBackward = moving_std(signal, stdWinSize, 'backward');
